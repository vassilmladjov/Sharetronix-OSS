<?php
	...
	function create_database($convert_version=FALSE)
	{
		...
		if( $v == "3.0.0" ) {
			$res = $res && my_mysql_query('CREATE TABLE IF NOT EXISTS `plugins_installed` (
			...
			$res = $res && my_mysql_query('CREATE TABLE IF NOT EXISTS `plugins_tables` (
					`id` int(11) NOT NULL AUTO_INCREMENT,
			...
			$res = $res && my_mysql_query("DROP TABLE IF EXISTS `languages`");
			$res = $res && my_mysql_query("CREATE TABLE IF NOT EXISTS `languages` (
			...
			$res = $res && my_mysql_query("TRUNCATE TABLE `plugins_tables`", $conn);
			...
			while($tbl = my_mysql_fetch_row($tmp)) {
				...
				if( substr($tbl, 0, strlen($prefix)) == $prefix ) {
					$res = $res && my_mysql_query("INSERT INTO `plugins_tables` (`table`, `owner`) VALUES ('".$tbl."', 'system')", $conn);
				}
			}
			
			$res = $res && my_mysql_query('SELECT id FROM `posts` WHERE user_id=0 AND api_id=0');
			while( $obj = my_mysql_fetch_row($res) ){
				$db2->query('DELETE FROM `post_userbox` WHERE post_id="'.$obj[0].'" LIMIT 1');
				$db2->query('DELETE FROM `posts` WHERE user_id=0 AND api_id=0 AND id="'.$obj[0].'" LIMIT 1');
			}
		...
		$res	= $res && my_mysql_query('CREATE TABLE IF NOT EXISTS  `plugins` (
		...
		$res	= $res && my_mysql_query('CREATE TABLE IF NOT EXISTS  `plugins_cache` (
			`id` INT( 6 ) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY ,
