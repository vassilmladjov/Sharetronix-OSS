<?php
	...
	{	
		...
		if(!$err)
		{
			...
			else
			{
				...
				{						
					$callback = $oauth_client->get_field_in_table('applications', 'callback_url', 'app_id', 
													$oauth_client->get_value_in_consumer_key('app_id'));
	...
	{	
		...
		<form method="post" action="<?= $C->SITE_URL.'oauth/authorize?oauth_token='.$_GET['oauth_token'] ?>">
