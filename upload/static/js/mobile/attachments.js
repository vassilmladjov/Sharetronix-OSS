...
var Attachments = function () {
    ...
    function swapLinkContainer() {
        $('body').click(function (event) {
            caller = event.target;
            if ($(caller).parents('.attachment-link-field-container').length == 0 && !$(caller).hasClass('attachment-button')) {
                $('.attachment-link-field-container').hide();
            }
        });

        $('.attachment-button.link').live('click', function () {
            var cnt = $(this).parent('.attachments-options').find('.attachment-link-field-container');
